...
class UserWindow:
    ...
    def perform_search(self):
        ...
        if usuario:
            ...
            self.mostrar_otro_usuario(usuario)
        else:
    ...
    def mostrar_otro_usuario(self, usuario):
        ...
        tk.Button(user_window, text="Enviar solicitud de amistad", command=lambda: self.enviar_solicitud(usuario)).pack(
            pady=10)

    def enviar_solicitud(self, usuario):
        # "User" es el usuario original y "usuario" es a quien se le envía la solicitud.
        self.graph.agregar_arista(self.user, usuario)
        print(f"Solicitud de amistad enviada a {usuario.nombre}")
        tk.Button(text="Enviada")
